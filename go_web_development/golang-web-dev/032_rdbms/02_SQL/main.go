...
func main() {
	//db, err = sql.Open("postgres", "awsuser:mypassword@tcp(mydbinstance.cakwl95bxza0.us-west-1.rds.amazonaws.com:3306)/test02?charset=utf8")
	db, err = sql.Open("postgres", "host=localhost port=5432 user=username dbname=db password=pass sslmode=disable")

	if err != nil{
		fmt.Println("ananen", err)
	}
	defer db.Close()
...
func amigos(w http.ResponseWriter, req *http.Request) {
	rows, err := db.Query(`SELECT name FROM ocs;`)
	check(err)
	defer rows.Close()
